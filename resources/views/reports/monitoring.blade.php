        ...
        <section class="silat-card">
            ...
            <div class="overflow-x-auto">
                <table class="silat-table">
                    <thead class="silat-table-head"><tr><th class="silat-table-cell">Mahasiswa</th><th class="silat-table-cell">NPM</th><th class="silat-table-cell">Email</th><th class="silat-table-cell">Mitra</th><th class="silat-table-cell">Presensi</th><th class="silat-table-cell">Durasi</th><th class="silat-table-cell">Jam</th><th class="silat-table-cell">Laporan</th><th class="silat-table-cell">Seminar</th><th class="silat-table-cell">Nilai</th><th class="silat-table-cell">Lupa Presensi</th><th class="silat-table-cell">Catatan Harian</th><th class="silat-table-cell">Sanksi</th></tr></thead>
                    <tbody class="divide-y divide-gray-100 bg-white text-gray-700">
                        @forelse ($rows as $row)
                            <tr>
                                @php($photoUrl = \App\Support\PublicStorage::url($row['photo_url']))
                                <td class="silat-table-cell"><div class="flex items-center gap-3">@if ($photoUrl)<img src="{{ $photoUrl }}" alt="" class="h-9 w-9 rounded-full object-cover">@else<div class="flex h-9 w-9 items-center justify-center rounded-full bg-gray-100 text-xs font-semibold text-gray-500">{{ Str::of($row['name'])->substr(0, 1)->upper() }}</div>@endif<div><p class="font-medium text-gray-900">{{ $row['name'] }}</p><p class="text-xs text-gray-500">{{ $row['study_program'] }} / {{ $row['period'] }}</p></div></div></td>
                        ...
                        @empty
                            <tr><td colspan="13" class="silat-table-cell"><x-empty-state title="Tidak ada data pada filter ini" icon="fa-chart-column" /></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
