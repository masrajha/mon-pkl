        ...
        <div class="grid gap-6 lg:grid-cols-[1fr_2fr]">
            ...
            <div class="silat-card overflow-hidden">
                ...
                <table class="silat-table">
                    ...
                    <tbody>@foreach ($enrollments as $enrollment)<tr><td class="silat-table-cell"><span class="font-medium text-gray-900">{{ $enrollment->student?->full_name }}</span><div class="text-xs text-gray-500">{{ $enrollment->student?->npm }}</div></td><td class="silat-table-cell">{{ $enrollment->internshipPeriod?->display_name }}<div class="text-xs text-gray-500">{{ $enrollment->studyProgram?->name }}</div></td><td class="silat-table-cell text-gray-600">{{ $enrollment->internshipPlace?->name ?: '-' }}</td><td class="silat-table-cell"><span class="font-medium text-gray-900">{{ $enrollment->lecturer?->name ?: $enrollment->lecturerSupervisor?->name ?: ($enrollment->lecturer_supervisor ?: '-') }}</span><div class="text-xs text-gray-500">{{ $enrollment->field_supervisor ?: '-' }}</div></td><td class="silat-table-cell">@php($statusVariant = match($enrollment->status) {'active' => 'success', 'pending_verification', 'draft' => 'warning', 'completed' => 'info', 'revision_required' => 'warning', 'cancelled', 'rejected' => 'danger', default => 'neutral'})<x-badge :variant="$statusVariant">{{ Str::headline($enrollment->status) }}</x-badge>@if($enrollment->admin_note)<div class="mt-1 text-xs text-amber-700">{{ Str::limit($enrollment->admin_note, 60) }}</div>@endif</td><td class="silat-table-cell text-right"><a class="silat-secondary-link justify-end" href="{{ route('management.enrollments.edit', ['enrollment' => $enrollment] + request()->only(['q', 'period_id', 'study_program_id', 'status', 'page', 'per_page', 'sort', 'direction'])) }}"><x-icon name="fa-clipboard-check" class="mr-1" /> Validasi/Edit</a></td></tr>@endforeach</tbody>
                </table>
