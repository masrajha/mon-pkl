        ...
        <div class="silat-card overflow-hidden">
            ...
            <div class="silat-table-wrap">
                <table class="silat-table">
                    ...
                    <tbody>@foreach ($places as $place)<tr><td class="silat-table-cell"><input data-place-checkbox type="checkbox" name="place_ids[]" value="{{ $place->id }}" class="rounded border-gray-300"></td><td class="silat-table-cell"><div class="font-medium text-gray-900">{{ $place->name }}</div><div class="text-xs text-gray-500">{{ $place->address }}</div></td><td class="silat-table-cell text-gray-600">{{ $place->city?->name ?: '-' }}</td><td class="silat-table-cell"><x-badge :variant="$place->is_active ? 'success' : 'neutral'">{{ $place->is_active ? 'Aktif' : 'Nonaktif' }}</x-badge></td><td class="silat-table-cell text-gray-600">{{ $place->latitude && $place->longitude ? $place->latitude.', '.$place->longitude : '-' }}</td><td class="silat-table-cell text-gray-600">{{ $place->enrollments_count }}</td><td class="silat-table-cell text-right"><a class="silat-secondary-link justify-end" href="{{ route('management.places.edit', $place) }}"><x-icon name="fa-pen-to-square" class="mr-1" /> Edit</a></td></tr>@endforeach</tbody>
                </table>
            </div>
            </form>
