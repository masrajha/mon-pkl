...
<x-app-layout>
    ...
    <div class="py-8"><div class="silat-shell space-y-6">
        ...
        <div x-data="{ activeTab: @js($initialTab) }" class="space-y-6">
            @if (session('suggest_forgotten_check_in') && $remainingForgottenAttendanceRequests > 0)
                <x-alert variant="warning">
                    Presensi pulang tersimpan tanpa pasangan masuk. Hari ini belum dihitung valid sampai presensi masuk dilengkapi.
                    <button type="button" class="ml-2 font-semibold text-blue-700 underline" @click="activeTab = 'forgotten'">Ajukan Lupa Presensi Masuk</button>
                </x-alert>
            @elseif (session('suggest_forgotten_check_in'))
                <x-alert variant="warning">
                    Presensi pulang tersimpan tanpa pasangan masuk. Kuota Lupa Presensi sudah habis, sehingga hari ini tetap belum valid sampai diverifikasi pengelola.
                </x-alert>
            @endif
                ...
                <div class="grid gap-6 xl:grid-cols-[1.6fr_0.9fr]">
                    <section class="silat-card overflow-hidden">
                        ...
                        <div class="p-5">
                            <div
                                ...
                                data-accuracy-input="student_location_accuracy"
                                data-sample-input="location_sample_id"
                                data-location-sample-url="{{ route('check-ins.location-samples.store') }}"
                    ...
                    <section class="silat-card">
                        <form method="POST" action="{{ route('check-ins.store') }}" class="space-y-5 p-5" x-data="{ action: '{{ old('action', 'check_in') }}' }">
                            ...
                            <input id="location_sample_id" name="location_sample_id" type="hidden" value="{{ old('location_sample_id') }}" required>
                            <div>
                            ...
                            <input id="student_location_accuracy" name="student_location_accuracy" type="hidden" value="{{ old('student_location_accuracy') }}">
                            <p id="location_sample_status" class="text-xs text-gray-500">Menunggu snapshot lokasi GPS tersimpan.</p>
                            <div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($recentCheckIns as $checkIn)
                                    <tr>
                                        ...
                                        <td class="silat-table-cell">{{ $checkIn->duration_minutes !== null ? floor($checkIn->duration_minutes / 60).'j '.($checkIn->duration_minutes % 60).'m' : '-' }}@if($checkIn->sanction_points)<div class="text-xs text-rose-600">{{ $checkIn->sanction_points }} poin</div>@endif</td>
                                        <td class="silat-table-cell">
                                            {{ $checkIn->distance_meters === null ? '-' : number_format($checkIn->distance_meters, 0, ',', '.').' m' }}
                                            @if($checkIn->student_location_accuracy_meters !== null)<div class="text-xs text-gray-500">Akurasi {{ number_format($checkIn->student_location_accuracy_meters, 0, ',', '.') }} m</div>@endif
                                        </td>
                            ...
                            <div class="space-y-4">
                                <div class="grid gap-4 sm:grid-cols-3">
                                    <div>
                                        ...
                                        <x-text-input id="forgotten_requested_date" name="requested_date" type="date" class="mt-1 block w-full" :value="old('requested_date', session('suggest_forgotten_check_in') ? $forgottenAttendanceSuggestedDate : null)" required />
                                    </div>
                                    <div>
