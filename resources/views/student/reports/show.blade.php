                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($dailyActivityRows as $row)
                                    ...
                                    <tr>
                                        ...
                                        <td class="silat-table-cell whitespace-nowrap"><div>Masuk: {{ $row['check_in']?->checked_at?->format('H:i:s') ?: '-' }} <x-device-badge :device="\App\Support\DeviceInfo::from($row['check_in']?->device_info)" /></div><div>Pulang: {{ $row['check_out']?->checked_at?->format('H:i:s') ?: '-' }} <x-device-badge :device="\App\Support\DeviceInfo::from($row['check_out']?->device_info)" /></div><div>Durasi: {{ $row['duration_minutes'] !== null ? number_format($row['duration_minutes'] / 60, 2, ',', '.') : '-' }}</div></td>
                        ...
                        <table class="silat-table">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($enrollment->checkIns as $checkIn)
                                    <tr><td class="silat-table-cell">{{ $checkIn->checked_at?->format('d/m/Y H:i') }}<div class="mt-1"><x-device-badge :device="\App\Support\DeviceInfo::from($checkIn->device_info)" /></div></td><td class="silat-table-cell">{{ $checkIn->action === 'check_out' ? 'Pulang' : 'Masuk' }}</td><td class="silat-table-cell"><x-badge>{{ $checkIn->type }}</x-badge>@if($checkIn->work_mode === 'wfa')<div class="mt-1"><x-badge variant="success">WFA</x-badge></div>@endif</td><td class="silat-table-cell">{{ $checkIn->duration_minutes !== null ? floor($checkIn->duration_minutes / 60).'j '.($checkIn->duration_minutes % 60).'m' : '-' }}@if ($checkIn->sanction_points)<div class="text-xs text-rose-600">{{ $checkIn->sanction_points }} poin</div>@endif</td><td class="silat-table-cell">{{ $checkIn->distance_meters !== null ? number_format($checkIn->distance_meters, 0, ',', '.').' m' : '-' }}</td><td class="silat-table-cell">{{ $checkIn->note ?: '-' }}</td></tr>
                                @empty
