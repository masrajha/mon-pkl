<x-app-layout>
    <x-slot name="header"><h2 class="text-xl font-semibold text-gray-800">{{ __('Validasi Usulan Tempat PKL') }}</h2></x-slot>
    <div class="py-10"><div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <form class="mb-4 bg-white p-4 shadow-sm sm:rounded-lg">
            <select name="status" class="rounded-md border-gray-300"><option value="">Semua status</option>@foreach (['pending','approved','merged','rejected'] as $status)<option value="{{ $status }}" @selected($selectedStatus === $status)>{{ $status }}</option>@endforeach</select>
        ...
        <div class="space-y-4">
                ...
                <div class="bg-white p-5 shadow-sm sm:rounded-lg">
                    <div class="flex flex-wrap justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ $proposal->student?->full_name }} · {{ $proposal->internshipPeriod?->name }} · {{ $proposal->studyProgram?->name }}</p>
            ...
            @empty
                <div class="bg-white p-6 text-center text-sm text-gray-500 shadow-sm sm:rounded-lg">Belum ada usulan tempat PKL.</div>
            @endforelse
        </div>
